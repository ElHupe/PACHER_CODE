...
for (i in directories){
  ...
  if (!file.exists(here::here(i))){
  dir.create(here::here(i)) 
  }
}
...
if (file.exists(here::here("libraries.txt"))){
  libraries <- unlist(read.table(here::here("libraries.txt"), stringsAsFactors = FALSE))
...
if (file.exists(here::here("Robjects.txt"))){
  Robjects <- unlist(read.table(here::here("Robjects.txt"), stringsAsFactors = FALSE))
  for (i in Robjects){
    load(i)
...
files <- dir(here::here(scriptsdir), pattern = "function", full.names = TRUE)
for (file in files){
  source(file)
}
rm(files, file)
